import { Button, Card, CardBody, Radio, RadioGroup, Spinner, Text, VStack } from '@chakra-ui/react'
import { useShopper } from '@ordercloud/react-sdk'
import { Address, OrderShipMethodSelection, ShipMethod } from 'ordercloud-javascript-sdk'
...
const CartShippingPanel: React.FC<CartShippingPanelProps> = ({ handleNextTab }) => {
  ...
  const handleSelectShipMethod = async () => {
    ...
    const shipEstimateID = orderWorksheet?.ShipEstimateResponse?.ShipEstimates?.at(0)?.ID
    ...
    const shipMethodSelection: OrderShipMethodSelection = {
      ShipMethodSelections: [{ ShipEstimateID: shipEstimateID, ShipMethodID: shipMethodID }],
    }
  ...
  if (error) {
    return (
      <Card>
        <CardBody>
          Shipping service unavailable, please contact your site admin and try again later
        </CardBody>
      </Card>
    )
  }
      ...
      >
        ...
        >
          <RadioGroup
            ...
            as={VStack}
          >
            {orderWorksheet?.ShipEstimateResponse?.ShipEstimates?.at(0)?.ShipMethods?.map(
              (method: ShipMethod) => (
                <Radio
                  key={method.ID}
                  value={method.ID}
