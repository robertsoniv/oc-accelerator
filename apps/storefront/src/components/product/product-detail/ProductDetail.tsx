...
import { BuyerProduct, InventoryRecord, OrderCloudError } from 'ordercloud-javascript-sdk'
import pluralize from 'pluralize'
import React, { useCallback, useEffect, useMemo, useState } from 'react'
import { useNavigate } from 'react-router-dom'
import OcQuantityInput from '@components/checkout/cart/CartQuantityInput'
import ProductImageGallery from '@components/product/product-detail/ProductImageGallery'
import { useOcResourceGet, useOcResourceList, useShopper } from '@ordercloud/react-sdk'
...
const ProductDetail: React.FC<ProductDetailProps> = ({ productId, renderProductDetail }) => {
  const navigate = useNavigate()
  const toast = useToast()
  const [activeRecordId, setActiveRecordId] = useState<string>()
  const { data: product, isLoading: loading } = useOcResourceGet<BuyerProduct>('Me.Products', {
    productID: productId,
  })
  ...
  const [quantity, setQuantity] = useState(product?.PriceSchedule?.MinQuantity ?? 1)
  ...
  useEffect(() => {
    const availableRecord = inventoryRecords?.Items.find((item) => item.QuantityAvailable > 0)
    if (availableRecord) {
                  ...
                  inventoryRecords?.Items.map((item) => (
                    ...
                    >
                      ...
                      >
                        ...
                        {item.Address.Street2 && <Text>{item.Address.Street2}</Text>}
                      ...
                      <CardFooter
                        ...
                        fontSize="xs"
                      >
                        {item.QuantityAvailable === 0 ? 'Out of stock' : 'Select This Store'}
                      </CardFooter>
                    </Button>
                  ))}
              </HStack>
